...
export default function BlogMVP() {
    ...
    //         const response = await fetch(`/posts/articles/${post.filename}`);
    //         if (!response.ok) {
        ...
        <>
            ...
            <Section>
                {/* <PostBodySection
                    ...
                    notebook={isNotebook && content}
                /> */}
                {content && content["cells"].map((block, index) => {
                    if (block.cell_type === "markdown") {
                        return (<MarkdownFormatter key={index} markdown={block.source.join("")} />);
                    } else if (block.cell_type === "code") {
                        return (
                            // <DelayedLoadPythonComponent
                            //     key={index}
                            //     content={block.content}
                            // />
                            <NotebookCell key={index} data={block} />
                        );
                    }
                })}
            </Section>
