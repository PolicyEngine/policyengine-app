...
import { posts, topicTags, locationTags, locationLabels, topicLabels } from "../data/Posts";
...
function ResearchExplorer() {
  ...
  const searchTools = <BlogPostSearchTools 
    onSearch={onSearch}
...
function BlogPostResults({ posts }) {
  ...
  if (displayCategory === "desktop") {
    postComponents = posts.map((post) => (
      <div key={JSON.stringify(post)} style={{
        width: "47%",
        marginBottom: 40,
      }}>
...
function BlogPostSearchTools({ onSearch, filteredTopics, setFilteredTopics, filteredLocations, setFilteredLocations }) {
  ...
  const filterTools = <>
    <p className="spaced-sans-serif" style={{
...
function ExpandableCheckBoxList({ title, keys, keyToLabel, checkedValues, setCheckedValues }) {
  return <Expandable title={title}>
    {
      keys.map(key => <Checkbox key={key} label={keyToLabel[key] || key} checked={checkedValues.includes(key)} onCheck={
        checkedValues.includes(key) ?
          () => setCheckedValues(checkedValues.filter(k => k !== key)) :
          () => setCheckedValues(checkedValues.concat([key]))
      } />)
    }
    <div style={{borderBottom: "1px solid lightgray", width: "100%", marginTop: 5, marginBottom: 5}} />
    <Checkbox label="Select all" checked={checkedValues.length === keys.length} onCheck={
...
function Expandable({ title, children }) {
  ...
  const titleComponent = <div 
      style={{display: "flex", alignItems: "center"}} ref={titleRef}
      onClick={() => setExpanded(!expanded)}
    >
    <p style={{margin: 0}}>{title}</p>
    <FontIcon name="arrow_drop_down" size={20} style={{
      marginLeft: "auto",
  ...
  return <motion.div
    ...
    animate={{
      maxHeight: expanded ?
        contentRef.current?.getBoundingClientRect().height + titleRef.current?.getBoundingClientRect().height :
    ...
    >
      {titleComponent}
      <div style={{paddingLeft: 20, paddingTop: 10}} ref={contentRef}>{children}</div>
    </motion.div>
}
...
function Checkbox({ label, checked, onCheck }) {
  ...
  return <div style={{display: "flex", alignItems: "center"}}>
    <input type="checkbox" title={label} style={{
      borderRadius: 0,
      ...
      margin: 5,
      appearance: "none",
      backgroundColor: checked ? style.colors.TEAL_PRESSED : style.colors.TEAL_LIGHT,
      cursor: "pointer",
    }}
    onClick={() => {
      onCheck(!checked)
    ...
    />
    <p style={{marginLeft: 15, margin: 0, fontFamily: "Roboto Serif" }}>{label}</p>
  </div>
}