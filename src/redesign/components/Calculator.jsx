...
export default function Calculator() {
    ...
    const results = parameterSearch === "" ? [] : fuse.search(parameterSearch).slice(0, 3);
    return <>
        <Sidebar />
        ...
        <Section title="1. Select policy parameters">
            ...
            title="Search for a parameter" placeholder="Search by keyword or policy number" titleColor="black" />
            {results.map(result => <div key={result.item.name} style={{marginTop: 10, alignItems: "center", display: "flex"}}>
                <a href="/" className="highlighted-link" style={{backgroundColor: style.colors.LIGHT_GRAY}}>{result.item.label}</a>
