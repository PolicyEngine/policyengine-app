...
describe('flattenTree Integration Tests', () => {
    ...
    test('should flatten POLICY_OUTPUT_TREE correctly for US', () => {
        ...
        expect(flattenedTree).toEqual(expect.arrayContaining([
            // Policy Breakdown
            expect.objectContaining({ name: 'policyOutput.policyBreakdown', label: 'Overview' }),
            // Budgetary Impact
            expect.objectContaining({ name: 'policyOutput.budgetaryImpact.overall', label: 'Overall' }),
            // Distributional Impact
            expect.objectContaining({ name: 'policyOutput.distributionalImpact.incomeDecile.relative', label: 'Relative' }),
            expect.objectContaining({ name: 'policyOutput.distributionalImpact.incomeDecile.average', label: 'Average' }),
            // Winners and Losers
            expect.objectContaining({ name: 'policyOutput.winnersAndLosers.incomeDecile', label: 'By income decile' }),
            // Poverty Impact (Regular)
            expect.objectContaining({ name: 'policyOutput.povertyImpact.regular.byAge', label: 'By age' }),
            expect.objectContaining({ name: 'policyOutput.povertyImpact.regular.byGender', label: 'By gender' }),
            expect.objectContaining({ name: 'policyOutput.povertyImpact.regular.byRace', label: 'By race' }),
            // Poverty Impact (Deep)
            expect.objectContaining({ name: 'policyOutput.povertyImpact.deep.byAge', label: 'By age' }),
            expect.objectContaining({ name: 'policyOutput.povertyImpact.deep.byGender', label: 'By gender' }),
            // Inequality Impact
            expect.objectContaining({ name: 'policyOutput.inequalityImpact', label: 'Inequality impact' }),
            // Labor Supply Impact
            expect.objectContaining({ name: 'policyOutput.laborSupplyImpact.earnings.overall.relative', label: 'Relative' }),
            expect.objectContaining({ name: 'policyOutput.laborSupplyImpact.earnings.overall.absolute', label: 'Absolute' }),
            expect.objectContaining({ name: 'policyOutput.laborSupplyImpact.earnings.byDecile.relative.total', label: 'Total' }),
            expect.objectContaining({ name: 'policyOutput.laborSupplyImpact.earnings.byDecile.relative.income', label: 'Income effect' }),
            expect.objectContaining({ name: 'policyOutput.laborSupplyImpact.earnings.byDecile.relative.substitution', label: 'Substitution effect' }),
            expect.objectContaining({ name: 'policyOutput.laborSupplyImpact.earnings.byDecile.absolute.total', label: 'Total' }),
            expect.objectContaining({ name: 'policyOutput.laborSupplyImpact.earnings.byDecile.absolute.income', label: 'Income effect' }),
            expect.objectContaining({ name: 'policyOutput.laborSupplyImpact.earnings.byDecile.absolute.substitution', label: 'Substitution effect' }),
            // AI Summary
            expect.objectContaining({ name: 'policyOutput.analysis', label: 'AI summary (experimental)' }),
            // Code Reproducibility
            expect.objectContaining({ name: 'policyOutput.codeReproducibility', label: 'Reproduce in Python' }),
        ]));
    ...
    test('should flatten POLICY_OUTPUT_TREE correctly for UK', () => {
        ...
        expect(flattenedTree).toEqual(expect.arrayContaining([
            ...
            // Budgetary Impact
            expect.objectContaining({ name: 'policyOutput.budgetaryImpact.overall', label: 'Overall' }),
            expect.objectContaining({ name: 'policyOutput.budgetaryImpact.byProgram', label: 'By program' }),
            // Distributional Impact
            expect.objectContaining({ name: 'policyOutput.distributionalImpact.incomeDecile.relative', label: 'Relative' }),
            expect.objectContaining({ name: 'policyOutput.distributionalImpact.incomeDecile.average', label: 'Average' }),
            expect.objectContaining({ name: 'policyOutput.distributionalImpact.wealthDecile.relative', label: 'Relative' }),
            expect.objectContaining({ name: 'policyOutput.distributionalImpact.wealthDecile.average', label: 'Average' }),
            // Winners and Losers
            expect.objectContaining({ name: 'policyOutput.winnersAndLosers.incomeDecile', label: 'By income decile' }),
            expect.objectContaining({ name: 'policyOutput.winnersAndLosers.wealthDecile', label: 'By wealth decile' }),
            // Poverty Impact (Regular)
            expect.objectContaining({ name: 'policyOutput.povertyImpact.regular.byAge', label: 'By age' }),
            expect.objectContaining({ name: 'policyOutput.povertyImpact.regular.byGender', label: 'By gender' }),
            // Poverty Impact (Deep)
...
describe('MobileBottomNavButtons Component', () => {
