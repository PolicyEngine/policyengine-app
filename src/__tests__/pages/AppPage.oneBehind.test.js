...
describe("OBBBA 'One Behind' Issue", () => {
  it("demonstrates the problem", () => {
    ...
    const simulateIframeClick_PROBLEMATIC = (newHousehold) => {
      ...
      console.log(`  Parent URL shows: ${parentUrlHistory[parentUrlHistory.length - 1]}`);
      console.log(`  One behind? ${iframeInternalState.household !== parentUrlHistory[parentUrlHistory.length - 1]}`);
    };
    ...
    expect(parentUrlHistory[parentUrlHistory.length - 1]).not.toBe(iframeInternalState.household);
  });
  ...
  it("shows how it should work", () => {
    ...
    const simulateIframeClick_CORRECT = (newHousehold) => {
      ...
      console.log(`  Iframe shows: ${iframeInternalState.household}`);
      console.log(`  Parent URL shows: ${parentUrlHistory[parentUrlHistory.length - 1]}`);
      console.log(`  In sync? ${iframeInternalState.household === parentUrlHistory[parentUrlHistory.length - 1]}`);
    };
    ...
    expect(parentUrlHistory[parentUrlHistory.length - 1]).toBe(iframeInternalState.household);
  });
