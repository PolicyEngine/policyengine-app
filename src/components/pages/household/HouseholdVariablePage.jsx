...
function HouseholdVariableEntityInput(props) {
  ...
  const inputValue =
    PolicyEngine.household[entityPlural][entityName][variable.name][timePeriod];
  const simulatedValue = PolicyEngine.getSimulatedValue(
    variable.name,
    timePeriod,
    entityName
  );
...
function HouseholdVariableEntity(props) {
  ...
  const possibleTimePeriods = Object.keys(
    PolicyEngine.household[entityPlural][entityName][variable.name]
  );
  return (
    <div>
...
export default function HouseholdVariablePage(props) {
  const PolicyEngine = useContext(PolicyEngineContext);
  const variableName =
    PolicyEngine.householdPage.split(".")[
      PolicyEngine.householdPage.split(".").length - 1
    ];
  const selectedVariable = PolicyEngine.metadata.variables[variableName];
  const entityPlural =
    PolicyEngine.metadata.entities[selectedVariable.entity].plural;
  const variablesInOrder = PolicyEngine.variablesInOrder;
  const index = variablesInOrder.indexOf(variableName);
  const nextVariable =
    PolicyEngine.metadata.variables[variablesInOrder[index + 1]];
  const isSimulated = !selectedVariable.isInputVariable;
  const possibleEntities = Object.keys(
    PolicyEngine.household[entityPlural]
  ).filter(
    (entity) =>
      PolicyEngine.household[entityPlural][entity][selectedVariable.name]
  );
  return (
    <>
      <div
      ...
      >
        <h1 style={{ marginBottom: 20 }}>
          What is your {selectedVariable.label}?
        </h1>
        ...
        {nextVariable && (
          <h4
            ...
            onClick={() =>
              PolicyEngine.setState({
                householdPage:
                  nextVariable.moduleName + "." + nextVariable.name,
              })
            }
          >
            &#8594; {capitalize(nextVariable.label)}
          </h4>
        )}
      </div>
    </>
