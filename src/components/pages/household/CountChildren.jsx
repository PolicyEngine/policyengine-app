...
export default function CountChildren(props) {
  ...
  const getCountChildren =
    PolicyEngine.country == "uk" ? getUKCountChildren : getUSCountChildren;
  const setCountChildrenInHousehold =
    PolicyEngine.country == "uk" ? setUKCountChildren : setUSCountChildren;
    ...
    <>
      ...
      >
        ...
        <h4
          ...
          onClick={() =>
            PolicyEngine.setState({
              householdPage: nextVariable.moduleName + "." + nextVariable.name,
            })
          }
        >
          &#8594;{" "}
          {capitalize(
            PolicyEngine.metadata.variables[variablesInOrder[0]].label
          )}
        </h4>
      </motion.div>
    </>
